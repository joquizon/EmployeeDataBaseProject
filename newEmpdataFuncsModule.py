...
def hiredate(mn,dn,yn,list,dns):
    ...
    def hireMonth(yn,mn,list):
        ...
        if hireMo == 'year':
            hireYear(int(yn),list)
            hireMonth(int(yn),int(mn),list)
        else:
            while True:
                try:
                    hireMonom = int(hireMo)
                    if int(hiredateset[2]) < int(yn) and len(hireMo)==2 and hireMonom> 0 and hireMonom < 13:
                        hiredateset[0]=hireMo
                        list[9]= hireMo
                        break
                    elif int(hiredateset[2]) == int(yn) and hireMonom<= int(mn) and len(hireMo)==2 and hireMonom> 0:
                        hiredateset[0]=hireMo
                        list[9]=hireMo
                        break
                    else:
